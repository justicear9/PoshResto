<div
    ...
    <div class="sticky bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
        ...
        <div class="h-auto pt-3 select-none text-center w-full">
            @if (!$orderID)
                {{-- New order (no DB order yet): make Bill & Payment the primary action. --}}
                <div class="flex gap-3 mb-3">
                    <button class="rounded bg-green-500 hover:bg-green-600 text-white w-full p-2 relative {{ $isPastTime ? 'opacity-50 cursor-not-allowed' : '' }}"
                        wire:click="saveOrder('bill', 'payment')"
                        wire:loading.attr="disabled" wire:loading.class="opacity-50"
                        @if($isPastTime) disabled @endif>
                        <span wire:loading.remove wire:target="saveOrder('bill', 'payment')">
            ...
            @elseif ($orderID && $orderDetail && $orderDetail->status !== 'draft')
                {{-- Existing non-draft order: keep Save as Draft as the primary action --}}
                <div class="flex gap-3 mb-3">
            ...
            @if (!$orderID || ($orderID && $orderDetail && $orderDetail->status == 'draft'))
                <div class="flex gap-3 mt-3">
                    ...
                    @if (!$orderID)
                        {{-- New order: put Save as Draft into the "middle" slot --}}
                        <button class="rounded bg-yellow-600 hover:bg-yellow-700 text-white w-full p-2 relative {{ $isPastTime ? 'opacity-50 cursor-not-allowed' : '' }}"
                            wire:click="saveOrder('draft')" wire:loading.attr="disabled"
                            wire:loading.class="opacity-50"
                            @if($isPastTime) disabled @endif>
                            <span wire:loading.remove wire:target="saveOrder('draft')">@lang('modules.order.saveAsDraft')</span>
                            <span wire:loading wire:target="saveOrder('draft')">
                    ...
                    @else
                        {{-- Existing draft: keep Bill & Payment in the middle slot --}}
